...
class Orchestrator:
    ...
    async def run(
        ...
        coverage = await self._coverage.check(query)
        ...
        if coverage.action in (CoverageAction.BUILD_NEW, CoverageAction.EXTEND_EXISTING):
            ...
            if wiki_articles:
                domain_embedding = await self._embed(domain_label + " " + " ".join(coverage_topics or []))
        ...
        if output.slm_model_id:
            ...
            raw_decomp = await self._adapters.generate(
            ...
            if sub_tasks:
                ...
                coverage = self._coverage.evaluate_routing_plan(routing_plan, coverage)
                output.coverage_action = coverage.action.value
            ...
            steps.append(OrchestratorStep(
                ...
                explanation=StepExplanation(
                    ...
                    confidence=coverage.max_confidence or 0.5,
        ...
        for task_type in task_types_needed:
            recs = await self._catalog.recommend(
                ...
                slm_model_id=output.slm_model_id,
            )
            for r in recs[:2]:
        ...
        steps.append(OrchestratorStep(
            ...
            explanation=StepExplanation(
                ...
                confidence=max((r.composite_score for r in all_recs), default=0.5),
        ...
        for st in sub_tasks:
            ...
            if not model_for_task:
                ...
                model_for_task = output.slm_model_id or (_best.model_id if _best else None)
            if not model_for_task:
                continue

            context_prefix = f"Knowledge Graph Context:\n{compressed_context}\n\n" if compressed_context else ""
            try:
                response = await self._adapters.generate(
                    model_for_task,
                    f"{context_prefix}Question: {fragment}",
                    temperature=0.2,
                )
            except Exception as exc:
                # Primary model unavailable (no API key / not installed) — fall back to local SLM
                _fallback_info = await self._adapters.get_best_local_model()
                fallback_model = output.slm_model_id or (_fallback_info.model_id if _fallback_info else None)
            ...
            sub_results.append(SubTaskResult(
                ...
                response=response,
                confidence=st.get("my_confidence", 0.5),
            ))

        if not sub_results:
            # No decomposition — direct domain answer
            _best = await self._adapters.get_best_local_model()
            model_for_task = output.slm_model_id or (_best.model_id if _best else None)
            if model_for_task:
                context_prefix = f"Knowledge Graph Context:\n{compressed_context}\n\n" if compressed_context else ""
        ...
        if synth_model and sub_results_text:
            ...
            try:
                final_answer = await self._adapters.generate(synth_model, synth_prompt, temperature=0.15)
            except Exception:
