...
export interface OrchestratorEvent {
  ...
  phase?: string;
  step?: number;
  step_name?: string;   // top-level stage name (new backend format)
  detail?: string;      // brief detail string from the stage
  [k: string]: unknown;
}
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        // Production backend sends type="output" with payload in ev.data;
        // demo mock sends type="final" with payload directly on ev.
        if (ev.type === "output") output = ev.data as OrchestratorOutput;
        if (ev.type === "final") output = ev as unknown as OrchestratorOutput;
        onEvent(ev);
