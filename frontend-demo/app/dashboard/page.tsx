...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      {/* ── Executive Overview ─────────────────────────────────────────────
          Introduces DHS before users begin. Sits ABOVE the existing dashboard.
          Everything below this comment is the original unchanged dashboard.
      ──────────────────────────────────────────────────────────────────── */}
      <div className="bg-card border-b border-dborder px-12 pt-10 pb-8">
        {/* Badge */}
        <div className="flex items-center gap-2 mb-5">
          <span className="inline-block w-4 h-px bg-accent" />
          <span className="text-[10px] font-bold uppercase tracking-[.12em] text-t3">
            C5i Domain Harness System &nbsp;·&nbsp; Four-layer Intelligence Pipeline
          </span>
        </div>

        {/* Heading */}
        <h1 className="font-sora text-[28px] font-bold leading-tight text-t1 mb-3">
          Your Organization.<br />Your Business Unit.<br />
          <span className="text-accent">Your Intelligence.</span>
        </h1>

        {/* Description */}
        <p className="text-[13px] text-t2 leading-relaxed max-w-2xl mb-7">
          Traditional AI answers questions. The <span className="font-semibold text-t1">Domain Harness System</span> transforms enterprise knowledge into organisation-specific intelligence by building a reusable Small Language Model (SLM). Across four harnessing stages, DHS ingests enterprise knowledge, constructs contextual intelligence, reasons over domain-specific information, and delivers measurable business outcomes.
        </p>

        {/* Four stage cards */}
        <div className="grid grid-cols-4 gap-3 mb-7">
          {([
            { icon: "📥", title: "Information Harnessing",  desc: "Acquire enterprise documents, databases and structured knowledge." },
            { icon: "🕸",  title: "Knowledge Harnessing",   desc: "Build the Knowledge Graph, Domain Wiki and Custom AI." },
            { icon: "🤖", title: "Inference Harnessing",    desc: "Run enterprise reasoning using the trained SLM." },
            { icon: "✨", title: "Outcome Harnessing",      desc: "Review answers, reasoning, recommendations and business insights." },
          ] as const).map((s, i) => (
            <div key={i} className="bg-bg2 border border-dborder rounded-xl p-4">
              <div className="text-xl mb-2">{s.icon}</div>
              <div className="text-[12px] font-semibold text-t1 mb-1">{s.title}</div>
        ...
        <div className="flex items-center gap-2 flex-wrap mb-5">
          {([
            { label: "Information Harnessing", path: "/" },
            { label: "Knowledge Harnessing",   path: "/processing" },
            { label: "Inference Harnessing",   path: "/query" },
            { label: "Benchmarking",           path: "/benchmarking" },
          ] as const).map(b => (
            <button
              key={b.path}
              onClick={() => router.push(b.path)}
              className="px-4 py-2 rounded-full text-[11px] font-semibold border border-dborder2 text-t2 bg-card hover:border-accent/50 hover:text-accent hover:bg-accent/5 transition-all"
            >
              {b.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {["Knowledge Graph", "Domain Wiki", "Custom AI", "Enterprise Reasoning", "Explainable AI", "Benchmark Analytics"].map(tag => (
            <span key={tag} className="inline-flex items-center gap-1 text-[10px] font-semibold text-gg">
              <span className="text-gg">✓</span> {tag}
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-card border-b border-dborder px-0 py-7 mb-7">
        <div className="w-full px-12">
          ...
          <div className="font-sora text-2xl font-semibold text-t1">Intelligence Overview</div>
                ...
                <div className="flex items-center gap-3 p-3 bg-card2 border border-dborder rounded-sm transition-colors hover:border-dborder2 cursor-pointer group" onClick={() => resumeSession(s)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-t3 mt-0.5 flex items-center gap-2 flex-wrap">
                      <span className="bg-bg4 border border-dborder px-1.5 py-0.5 rounded text-[9px]">{s.project_name || s.domain_label}</span>
